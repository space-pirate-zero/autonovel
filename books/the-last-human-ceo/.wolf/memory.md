...
## 2026-06-24 — built print KDP assets
Rebuilt interior.pdf (341pp, 5.5×8.5in) + cover.pdf (12.1025×8.75in wraparound, spine 0.8525in) from current chapters. All fonts embedded in both. Flow: `python3 typeset/build_kdp.py --pages 341`, `tectonic interior.tex`, verify 341pp, `tectonic cover.tex`. Updated anatomy print constants 325→341pp.

## 2026-06-24 — fixed KDP cover size-mismatch (rebuilt 6x9 white)
KDP rejected cover: expected 13.018x9.250 vs submitted 12.102x8.750. Decoded expected size -> listing is 6x9 white (not 5.5x8.5 cream). Changed build_kdp.py constants (TRIM 6x9, PAPER white, SPINE 0.002252). Rebuilt: interior reflowed 341->295pp; cover now 12.914x9.250 (spine 0.6643). Fonts embedded in both. Re-upload BOTH to KDP. Logged to buglog.json + cerebrum.md.

## 2026-06-29 — KDP rebuild at 6×9 white (295pp) + fixed metadata-clobber bug
Full rebuild from current chapters via tectonic (engine = tectonic, run from kdp/):
- `python3 typeset/build_kdp.py` → interior.tex; `cd kdp && tectonic interior.tex` → 295pp, 432×648pt (6×9), fonts embedded.
- `python3 typeset/build_kdp.py --pages 295 --epub` → cover.tex/metadata/ebook-cover.jpg(1800×2700)/epub; `tectonic cover.tex` → 1pg 929.83×666pt (12.914×9.250in), spine 0.6643in, all fonts embedded. epubcheck CLEAN (0/0/0/0).
- BUG FIXED in build_kdp.py main(): else-branch always ran write_metadata(0) when --epub absent, clobbering the correct --pages metadata back to 0 (this is why committed metadata.md showed pages=0/spine 0). Now write_metadata(0) only runs when `not pages`; build_epub gated separately. Logged to buglog.json.
- ebook-cover px line in metadata.md now computed from TRIM (was hardcoded 1650×2550).
- Committed sources: kdp/*.tex, metadata.md, build_kdp.py. Binaries gitignored.
