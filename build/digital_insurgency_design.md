```{=latex}
\dichapter{PROLOGUE}{THE PORTRAIT IN THE SERVER ROOM}{}
```
...
```{=latex}
\dichapter{CHAPTER 1}{The Rewrite}{Every Line of Code on Earth Has an Expiration Date}
```
...
```
R_extinction = (Lines_generic / Lines_authentic) x AI_velocity
```
...
\textcolor{acid}{GREEN} **GREEN (R_ext < 2):** Your authentic core outweighs your generic mass. The Rewrite makes you faster, not extinct. You're holding ground worth holding.

\textcolor{amber}{YELLOW} **YELLOW (2 <= R_ext < 5):** You're mostly building what anyone could build, propped up by a thin layer of real craft. The model is coming for the bulk of it. Decide now which lines you'd fight for.

\textcolor{red}{RED} **RED (R_ext >= 5):** You are landfill with a logo. A current model regenerates your product over a long weekend. The Graveyard has a storefront with your name in the window vinyl already. Move.

**Field Report Alpha --- Lumence (deceased).**
A "smart scheduling" SaaS. Of 240,000 lines, the engineering lead admitted maybe 4,000 carried real judgment --- a clever conflict-resolution heuristic the founder built after years running clinics. The rest: standard calendar plumbing. Lines_generic = 236,000. Lines_authentic = 4,000. Ratio = 59. Multiply by AI_velocity 1.0 -> **R_ext = 59. \textcolor{red}{RED} Deep red.** The 4,000 good lines drowned in 236,000 anyone could write. The model didn't kill Lumence. The 236,000 did. The model just sent the invoice.

**Field Report Bravo --- a niche logistics tool that lived.**
20,000 lines. But 9,000 encoded a specific customs-clearance dance the founder learned hauling freight for a decade --- rules no model has seen because they live in nobody's training data, only in his scar tissue. Lines_generic = 11,000. Lines_authentic = 9,000. Ratio = 1.22. AI_velocity 1.0 -> **R_ext = 1.22. \textcolor{acid}{GREEN} Green.** Small company. Boring website. Still shipping. The Rewrite handed his team the boring 11,000 for free and let them pour everything into the 9,000 nobody else has.

**Field Report Charlie --- your "AI-powered" feature.**
A 40-person startup bolted a chatbot onto a generic CRM. They counted 180,000 generic lines, 6,000 they'd call their own --- mostly the chatbot prompts, which a competitor copied in a week. Lines_authentic, honestly assessed, ~ 1,500. Ratio = 120. AI_velocity, because they're in the most-copied category on earth, call it 1.5 -> **R_ext = 180. \textcolor{red}{RED} Catastrophic.** They raised on the chatbot. The chatbot was the part with the shortest half-life. Which brings us to the second instrument.
...
\textcolor{acid}{GREEN} **GREEN (A_hl >= 4):** Your work resists the copy. Months to years before anyone --- human or model --- reproduces it. Defensible. Build here.

\textcolor{amber}{YELLOW} **YELLOW (1.5 <= A_hl < 4):** Some moat, eroding. Weeks to months of lead. Real but rented. Deepen the context or watch it close.

\textcolor{red}{RED} **RED (A_hl < 1.5):** No moat. Days. You're standing on sand at low tide. Either find the part of your work that doesn't compress, or accept you're decorating a portrait somebody else can repaint.

**Field Report Delta --- the freight tool again.**
Taste 7, Context 10 (a decade of scar tissue), Craft 7 -> numerator 24. Replicability 2 --- a model can't copy what it's never seen, and it's never seen one founder's customs scars. **A_hl = 12. \textcolor{acid}{GREEN} Deep green.** This is what survival looks like from the inside: unglamorous, specific, impossible to prompt.

**Field Report Echo --- the chatbot startup.**
Taste 5, Context 3, Craft 4 -> numerator 12. Replicability 9 --- a competitor cloned the prompts in a week. **A_hl = 1.33. \textcolor{red}{RED} Red.** They had a four-month head start and called it a moat. It was a puddle.

**Field Report Foxtrot --- a designer's component library.**
Taste 9, Context 6, Craft 8 -> numerator 23. Replicability 5 --- copyable, but the *taste* in the spacing and motion is the hard part, and most people don't even see it to copy it. **A_hl = 4.6. \textcolor{acid}{GREEN} Green, barely.** Notice: high taste can carry a moderate replicability. The fingerprint is doing the work.
...
```{=latex}
\dichapter{CHAPTER 2}{The Physics of Stagnation}{The Gravity Well}
```
...
**F_g = (Sunk_Cost x Political_Capital) / Migration_Effort^2**
...
\textcolor{acid}{GREEN} **GREEN (F_g < 1.0):** The vendor's hold is weak. You can displace head-on.
\textcolor{amber}{YELLOW} **YELLOW (1.0--5.0):** Coexist, don't replace. Be additive. Sit next to the incumbent, not on top of it.
\textcolor{red}{RED} **RED (> 5.0):** Frontal assault is suicide. You bypass entirely or you go home.
...
F_g = (40,000,000 x 9) / 8,000,000^2 = 360,000,000 / 64,000,000,000,000 = **0.0000056.**
Wait --- read it correctly with consistent units (millions): (40 x 9) / 8^2 = 360 / 64 = **5.6.** \textcolor{red}{RED} RED. Do not try to replace the ledger. Sit beside it. (This is why Ch 13 takes 19 months and never once touches the core.)

**Field Report B --- A marketing team's email tool.**
Sunk_Cost = $0.2M. Political_Capital = 2. Migration_Effort = $0.3M.
F_g = (0.2 x 2) / 0.3^2 = 0.4 / 0.09 = **4.4.** \textcolor{amber}{YELLOW} YELLOW. Smaller than the ledger, still sticky. Coexist first, displace later.

**Field Report C --- A dead internal wiki nobody updates.**
Sunk_Cost = $0.05M. Political_Capital = 1. Migration_Effort = $0.1M.
F_g = (0.05 x 1) / 0.1^2 = 0.05 / 0.01 = **5.0.** \textcolor{red}{RED} Surprising, right? A worthless wiki with near-zero sunk cost still scores red, because migration effort is tiny *and squared* --- the denominator can't save you when it's below one. Lesson: low effort doesn't mean low gravity. Sometimes the junk is glued down hardest because moving it is somehow still annoying and nobody's career depends on it being good.

### Intel Block 2 --- The Entropy Tax (E_tax)

Every system rots. The bill compounds.

**E_tax = Maintenance x e^(λ|t)**
...
\textcolor{acid}{GREEN} **GREEN (< 1.5x base):** Manageable. The tax is annoying, not fatal.
\textcolor{amber}{YELLOW} **YELLOW (1.5--3x):** The system is eating budget that should be building things.
\textcolor{red}{RED} **RED (> 3x):** The maintenance tax exceeds the cost of replacement. They're paying rent on a corpse.
...
E_tax = 1.2 x e^(0.3 x 6) = 1.2 x e^1.8 = 1.2 x 6.05 = **$7.26M.** \textcolor{red}{RED} They are paying six times the base to keep a job alive that one good engineer could rewrite in a quarter. That's your opening --- not to replace it, to *quantify the bleeding* so someone with budget finally feels it.

**Field Report B --- A documented internal API.**
Maintenance = $0.4M/yr. λ = 0.1. t = 3 years.
E_tax = 0.4 x e^0.3 = 0.4 x 1.35 = **$0.54M.** \textcolor{acid}{GREEN} Fine. Leave it alone. Don't go hunting for fights in green zones; you'll lose your credibility on a problem nobody feels.
...
\textcolor{acid}{GREEN} **GREEN (< 0.5):** Recent, well-documented. Low resistance.
\textcolor{amber}{YELLOW} **YELLOW (0.5--2):** Touch carefully.
\textcolor{red}{RED} **RED (> 2):** Nobody alive fully understands it. Every change is a coin flip with the lights off.

**Field Report A:** A system last updated 8 years ago, 12 pages of docs. μ_leg = 8/12 = **0.67.** \textcolor{amber}{YELLOW} Touch carefully --- there's at least a map, even if it's stained.
**Field Report B:** A system updated 5 years ago, 0 pages of real documentation (1 page of a README that says "TODO"). μ_leg = 5/1 = **5.0.** \textcolor{red}{RED} This is tribal knowledge in human skulls, and the skulls are walking out the door at retirement age. Don't refactor it. Wrap it. Build a clean layer around the monster and never open the cage.
...
\textcolor{acid}{GREEN} **GREEN (< 1.0):** They're winning. The codebase shrinks its problem set.
\textcolor{amber}{YELLOW} **YELLOW (1.0--1.5):** Treading water. Velocity is leaking into firefighting.
\textcolor{red}{RED} **RED (> 1.5):** The debt is compounding faster than anyone services it. The team isn't building. They're triaging a bleed.

**Field Report A:** A team finds 80 bugs a sprint, fixes 50. D_comp = 80/50 = **1.6.** \textcolor{red}{RED} They will tell you they're "almost stable." They are not. They are losing 30 bugs of ground every two weeks, forever, and they know it the way a smoker knows.
**Field Report B:** A disciplined team finds 20, fixes 28 (burning down a backlog). D_comp = 20/28 = **0.71.** \textcolor{acid}{GREEN} These people are not your customers. They don't have a wound. Move on.
...
\textcolor{acid}{GREEN} **GREEN (n <= 3, F_cp <= 3):** A decision can actually happen.
\textcolor{amber}{YELLOW} **YELLOW (n = 4--6, F_cp 6--15):** Slow. Survivable. Map every channel.
\textcolor{red}{RED} **RED (n >= 7, F_cp >= 21):** Paralysis. The group exists to not decide. Find the one person who can act alone, or you'll grow old in this conference room.

**Field Report A:** Your deal needs sign-off from 7 people. F_cp = 7x6/2 = **21.** \textcolor{red}{RED} Twenty-one channels, any one of which routes to "let's circle back next quarter." Your job is not to convince twenty-one people. Your job is to find the one with the bleeding neck and a signing authority, and bypass the other six.
**Field Report B:** A budget-holder with a wound and one rubber-stamp boss. n = 2. F_cp = 2x1/2 = **1.** \textcolor{acid}{GREEN} This is the whole game. One channel. Go.
...
\textcolor{acid}{GREEN} **GREEN (< 1):** They ship more than they announce. Rare. Trustworthy.
\textcolor{amber}{YELLOW} **YELLOW (1--3):** Normal corporate theater. Workable.
\textcolor{red}{RED} **RED (> 3):** A company that performs progress instead of shipping it. The "innovation lab" with eleven demo days and zero things in production. Don't sell here --- you'll spend a year being a slide in someone else's keynote.

**Field Report:** An "innovation team" with 6 press releases, 4 demo days, 1 production deploy in a year. R_t = (6+4)/1 = **10.** \textcolor{red}{RED} This is a theater company that bought a stage and called it a roadmap. Diogenes would walk in with his lamp and find nobody home.
...
He defends the dead investment not because he believes in it. Because abandoning it means standing up in a room and explaining why he spent $40M on a corpse. The Entropy Tax can hit 6x and he will keep paying it, because the tax is invisible and the admission is not.
...
```{=latex}
\dichapter{CHAPTER 3}{The Curator's Edge}{Why Unique Humans Are the New Gatekeepers}
```
...
**V_curator = (Signal_selected / Noise_available) x Context_depth**
...
**Benchmarks:**
- \textcolor{acid}{GREEN} GREEN: V_curator >= 5.0 --- your taste is a moat. People follow your "that one."
- \textcolor{amber}{YELLOW} YELLOW: 1.5 <= V_curator < 5.0 --- useful, but you're forwarding too much. Cut harder.
- \textcolor{red}{RED} RED: V_curator < 1.5 --- you're a second pipe of noise. The machine does this for free.

**Field Report A --- the newsletter that won by deleting.**
A founder ran a developer newsletter. Old version: 40 links a week, hand-grabbed from a feed of maybe 2,000 articles, Context_depth a generous 4 (she'd shipped real systems). V_curator = (40 / 2000) x 4 = 0.08. \textcolor{red}{RED} RED. She was a pipe. Open rates dying. She killed it down to *three* links a week, each one she'd personally read twice and could explain why it mattered, drawn from the same 2,000. New math: (3 / 2000) x 4 = 0.006 --- which looks *worse* until you fix the variable that actually moved: by reading deeper and adding a paragraph of why-this-matters per link, her Context_depth jumped to 9, and "Signal" stopped meaning "links forwarded" and started meaning "decisions a reader made because of her." Reframed honestly --- 3 genuinely actioned items out of a curated short list of 12 she considered: (3/12) x 9 = **2.25**. \textcolor{amber}{YELLOW}. Then she went to one link a week, the single best thing she read, with 400 words on why: (1/4) x 10 = **2.5** climbing toward green as readers learned her taste was reliable. Subscribers tripled. She was forwarding less and worth more.

**Field Report B --- the AI ops "summarizer" nobody needed.**
A team shipped an internal tool that auto-summarized every Slack channel each morning. Signal_selected: it surfaced ~50 "important" messages a day. Noise_available: 5,000. Context_depth: 1 --- it had none; it ranked by reactions and length. V_curator = (50/5000) x 1 = **0.01**. \textcolor{red}{RED}. People stopped reading the summary within a week because it was just shorter noise. Compare the one senior engineer who, unpaid and grumbling, dropped *two* messages into a "read this" channel each morning --- the two that would bite you by Friday --- out of the same 5,000, with 12 years of scar tissue (Context_depth 9): (2/5000) x 9 --- again the wrong frame. Reframe to her real funnel: 2 surfaced out of ~15 she flagged worth a look: (2/15) x 9 = **1.2**, climbing as the team learned her two-a-day were load-bearing. The free machine scored 0.01. The grumbling human scored a hundred times higher.
...
**T_authentic = Trust_human x (1 / Substitutability_AI)**
...
**Benchmarks:**
- \textcolor{acid}{GREEN} GREEN: T_authentic >= 30 --- you are a name people seek by name.
- \textcolor{amber}{YELLOW} YELLOW: 10 <= T_authentic < 30 --- real, but exposed; the river is rising toward you.
- \textcolor{red}{RED} RED: T_authentic < 10 --- the machine is already eating your lunch; you just haven't checked the table.

**Field Report C --- two consultants, same deck.**
Consultant One sells "AI strategy." Trust_human 6 (decent track record). But his deliverable is a slide template anyone can prompt a model into in nine minutes --- Substitutability 0.85. T = 6 x (1/0.85) = **7.06**. \textcolor{red}{RED} RED. He's about to get underpriced by a chatbot and he's mad about it on LinkedIn. Consultant Two sells the same engagements but she sits in the room, names the political faction nobody will say out loud, and tells the CIO the migration he's been promised is a fantasy --- using fifteen years she can't download. Trust_human 8, Substitutability 0.2. T = 8 x (1/0.2) = **40**. \textcolor{acid}{GREEN}. Same market, same price card. One is a printout. One is a person. The machine can't be in the room being right about the things that test badly.

**Field Report D --- the support team that signed its name.**
A SaaS company's support replies were AI-generated, fast, perfect, and trusted by exactly nobody --- customers smelled the river. Trust_human 3, Substitutability 0.95. T = 3 x (1/0.95) = **3.16**. \textcolor{red}{RED}. They ran an experiment: one human rep, replies slower, occasionally wrong, but signed with a real name and a real "honestly that's a bug, here's the ticket, here's my Tuesday." Trust_human climbed to 7 over a quarter; Substitutability dropped to 0.4 because customers were buying *her*, not the answer. T = 7 x (1/0.4) = **17.5**. \textcolor{amber}{YELLOW} and rising. Slower, wronger, worth five times more.
...
1. **Compute your own T_authentic this week.** Score your Trust_human honestly (ask three people who'd tell you the truth) and your Substitutability (have a model attempt your last deliverable; time it; judge it). Write both numbers down. If you land \textcolor{red}{RED}, you are not in a branding problem, you are in a survival problem.
...
```{=latex}
\dichapter{CHAPTER 4}{The Psychology of the Blockade}{Fear \& Factions}
```
...
**RT = (P_f x C_d) + (T_panic x C_career)**

- **RT** --- Risk Tax: the invisible surcharge your prospect adds to your price.
- **P_f** --- Probability of failure, as they estimate it (often 5--10x your estimate).
...
- \textcolor{acid}{GREEN} **RT < 1.5x C_d** --- Their fear is manageable. Normal selling works.
- \textcolor{amber}{YELLOW} **RT 1.5x--3x C_d** --- Fear dominates value. You must lower P_f and T_panic before price means anything.
- \textcolor{red}{RED} **RT > 3x C_d** --- They are not buying software, they are buying a resignation letter. No discount fixes this. Re-scope to shrink the blast radius or walk.
...
RT = (0.30 x $40K) + (4 x $250K) = $12K + $1,000,000 = **$1,012,000**.
That's a 25x tax on a $40K deal. \textcolor{red}{RED} Catastrophic. No wonder he flinched at "transform" --- you were quoting him a million dollars of personal exposure and calling it forty grand.
...
RT = (0.08 x $40K) + (1 x $20K) = $3,200 + $20,000 = **$23,200**. Now it's 0.58x the deal. \textcolor{acid}{GREEN} You didn't change the product. You changed the math in his head.
...
RT = (0.50 x $180K) + (5 x $2M) = $90K + $10,000,000 = **$10.09M**. 56x the deal. \textcolor{red}{RED} This is why nineteen months. You don't sell into a 56x tax. You spend a year and a half grinding P_f and T_panic down with proof until the number is survivable.
...
RT = (0.15 x $9K) + (1 x $5K) = $1,350 + $5,000 = **$6,350**. 0.7x the deal. \textcolor{acid}{GREEN} Sell normally. This is why your first beachhead is always the low-panic desk, not the executive war room.
...
**BENCHMARKS:**
- \textcolor{acid}{GREEN} **DF < 5** --- Fresh enough to consider a real "yes." Book your hard asks here.
- \textcolor{amber}{YELLOW} **DF 5--10** --- They'll defer. Expect "let me think about it," which means no.
- \textcolor{red}{RED} **DF > 10** --- Reptile mode. They will reject anything that requires imagination. Reschedule. You cannot win this slot.
...
DF = 40 / 4 = **10**. \textcolor{red}{RED} Right at the cliff. This is the real reason ZERO didn't open the forty-slide deck. Forty slides is forty more decisions on an empty tank. She made the meeting *easier*, not richer --- one reframe, one reversibility promise, one number. She lowered their cognitive cost instead of raising her information output.

**Field Report B --- Same people, 9:15 AM, next week.**
Decisions_made ~ 3. Glucose_remaining ~ 9.
DF = 3 / 9 = **0.33**. \textcolor{acid}{GREEN} This is where you ask for the signature. Move your closing meetings to the morning. It feels superstitious. It is biochemistry.

**Field Report C --- The "quick sync" at 5:30 PM Friday.**
Decisions_made ~ 55. Glucose_remaining ~ 2.
DF = 55 / 2 = **27.5**. \textcolor{red}{RED} Anyone who books you here is sabotaging you, knowingly or not. Decline it. "No" is the only thing that brain can produce, and they will produce it about you.
...
2. **Compute RT for your lead skeptic.** Use their inflated P_f, not yours. Multiply T_panic by their real C_career. If RT exceeds 3x your quote, do not discount --- re-scope to shrink the blast radius until the tax is survivable.
...
```{=latex}
\dichapter{CHAPTER 5}{The Portrait of Dorian App}{Digital Ethics and the Soul-Cost of Building the Wrong Thing}
```
...
> **D_gray = (Revenue_dark-pattern / Trust_eroded) x Time**
...
- \textcolor{acid}{GREEN} **GREEN (D_gray < 1.0):** Negligible dark-pattern revenue, trust intact. The portrait is a photograph, not a corpse.
- \textcolor{amber}{YELLOW} **YELLOW (1.0 -- 5.0):** Manipulation is now a line item. Trust is measurably bleeding. You have time to refinance this. Not much.
- \textcolor{red}{RED} **RED (> 5.0):** The business now *depends* on the rot. Killing the dark pattern would miss the number, so nobody will. This is MirrorMatch. This is most of the apps on your phone.
...
D_gray = (1.4 / 0.18) x 4 = **31.1.** Deep red. The number that walked ZERO to the sink.
...
D_gray = (0.09M / 0.4) x 2 = **0.45.** Green-ish --- but watch the trend line, because the badge teaches users to distrust *every* number you show them, and that bill arrives late.
...
D_gray = (0.6 / 0.5) x 3 = **3.6.** Yellow, climbing toward red. The FTC reads dashboards too.
...
> **S_soul = Σ(Impact_human x Intent_honest)**
...
- \textcolor{acid}{GREEN} **GREEN (S_soul > +10):** The portfolio nets out helping humans, honestly. Rare. Cherish it.
- \textcolor{amber}{YELLOW} **YELLOW (−10 to +10):** Mixed. Some features heal, some extract, and you're telling yourself the heal cancels the extract. It doesn't sum that way emotionally --- it does mathematically, which is worse, because you can see it.
- \textcolor{red}{RED} **RED (< −10):** You are a net-negative force on human lives and dressing it as growth. The Dorian Gray engine. Get out or change it.
...
Guided sessions: +4 x 0.9 = +3.6. Sleep stories: +3 x 0.9 = +2.7. Gentle reminders: +2 x 0.8 = +1.6. One sin --- a "your streak is in danger!" guilt push: −3 x 0.2 = −0.6.
...
Every mechanic optimizes toward the user re-borrowing. Instant-approval dopamine: −4 x 0.1 = −0.4. Auto-rollover default: −5 x 0.05 = −0.25. "You qualify for more!" upsell: −5 x 0.1 = −0.5. Friendly mascot: −2 x 0.1 = −0.2. The honesty multiplier is so low that even the catastrophic impacts read as small *per-feature* --- but stack ten of them across ten million installs and Σ is a crater.
...
```{=latex}
\dichapter{CHAPTER 6}{Architecture as Camouflage}{Inception Engineering}
```
...
**Benchmarks:**
- \textcolor{acid}{GREEN} **R_r <= 1.0** --- Small blast, fast rollback. Approvable. You are reversible faster than you are dangerous. Lead with this number.
- \textcolor{amber}{YELLOW} **R_r 1.0--3.0** --- Investigate. Either shrink the blast (scope down, feature-flag, shadow-mode) or speed the rollback (one-click revert, dark launch) before you present.
- \textcolor{red}{RED} **R_r > 3.0** --- Stop. Redesign. You are asking the ARB to bet a career on a thing you can't un-ring. They will say no, and they will be right.
...
R_r = 3 / 9 = **0.33.** \textcolor{acid}{GREEN} Deep green. She doesn't lead the ARB pitch with "AI." She leads with "this runs in shadow mode, takes zero actions, and I can kill it from my phone." The ARB exhales. The ARB approves the kill switch and the architecture rides in underneath it.
...
R_r = 10 / 1 = **10.0.** \textcolor{red}{RED} Catastrophic. This is the architecture every junior engineer proposes and every ARB correctly executes in the parking lot. The fix isn't a better pitch. It's the Strangler Fig: route ten percent of logins to the new system, keep the old one warm, roll back instantly if the error rate twitches. That redesign drops blast to 4 and lifts rollback to 9 --- R_r = 0.44, \textcolor{acid}{GREEN}. Same goal. Reversible.
...
R_r = 8 / 1 = **8.0.** \textcolor{red}{RED} The most dangerous changes are the ones that touch humans directly, because humans don't have a rollback button. Shrink the blast: send to a 5,000-user holdout first, measure complaint rate, *then* decide. Blast drops to 3, rollback (stop the next batch) lifts to 8. R_r = 0.38, \textcolor{acid}{GREEN}.
...
1. **Compute R_r before you present, not after.** Score your blast radius and rollback speed on the 1--10 scales. If you land \textcolor{red}{RED}, do not pitch --- redesign for reversibility first. The number is your opening line.
...
```{=latex}
\dichapter{CHAPTER 7}{The Art of the Wedge}{Winning the Beachhead}
```
...
**V_w = (Pain_acute x Trust_increment) / Friction_adopt**
...
\textcolor{acid}{GREEN} **GREEN (V_w >= 4.0):** The wedge spreads on its own. People show their neighbors. You stop selling and start fielding requests. Land-and-expand is happening to you.

\textcolor{amber}{YELLOW} **YELLOW (V_w 1.5--4.0):** It spreads, but only when you push. The pain is real but the friction is eating you, or the trust isn't compounding. Cut friction before you do anything else.

\textcolor{red}{RED} **RED (V_w < 1.5):** It does not spread. You've either solved a paper cut or built a kickoff meeting. Either way, the foco dies in isolation. Redesign.
...
V_w = (9 x 0.9) / 1 = **8.1.** \textcolor{acid}{GREEN} Deep green. Within three weeks two more compliance analysts ask Sarah where they can get "the button." The wedge spreads by mouth, never by sale. IT still doesn't know it exists.
...
V_w = (4 x 0.5) / 8 = **0.25.** \textcolor{red}{RED} Red. It does not spread. Six months later it's a tab nobody opens. They solved a want, not a wound, and buried it under friction.
...
V_w = (6 x 0.7) / 2 = **2.1.** \textcolor{amber}{YELLOW} Yellow. It spreads when championed, stalls when not. The dev cuts friction further --- auto-installs on channel creation --- and trust climbs as it proves reliable. Recalculated months later: (7 x 0.85) / 1 = 5.95. \textcolor{acid}{GREEN} The fix was never more features. It was less friction.
...
4. **Calculate V_w before you install.** Score Pain_acute, Trust_increment, Friction_adopt. If you're not \textcolor{acid}{GREEN} (>=4.0), cut friction until you are. Never install a red wedge.
...
**Exercise 3 --- Score Your Wedge.** Before you build, run the V_w numbers on the idea. Score Pain_acute (1--10), Trust_increment (0--1), Friction_adopt (1--10). Calculate V_w = (Pain x Trust) / Friction. Red? Don't build it yet --- list every source of friction and kill them one at a time until you cross 4.0. The most common fix is never "more features." It's "fewer steps."
...
```{=latex}
\dichapter{CHAPTER 8}{Good Trouble}{How to Be John Lewis Inside a Fortune 500}
```
...
**GT = (Impact_change x Alignment_values) / (Risk + Friction)**
...
**Benchmarks:**
- \textcolor{acid}{GREEN} GT >= 4.0 --- High-conversion trouble. The change is big, you believe in it, and the cost is survivable. Make this trouble.
- \textcolor{amber}{YELLOW} GT 2.0--3.9 --- Marginal. Worth it only if you can lower the denominator --- find a cover policy, shrink the blast radius, cut the friction. Re-engineer before you commit.
- \textcolor{red}{RED} GT < 2.0 --- Bad trouble. Either the impact is theater, or you don't really believe in it, or the risk will end you. Walk away. Save the courage.

**Field Report A --- Keisha's tool, the way she shipped it.**
Impact_change: 9 (two million a year, real). Alignment_values: 10 (she believed in it completely). Risk: 8 (unilateral, touched production, no cover). Friction: 7 (ARB would have killed or delayed it). GT = (9 x 10) / (8 + 7) = 90 / 15 = **6.0.** \textcolor{acid}{GREEN} on the numerator --- but read it again. The *idea* scored green. The *execution* drove Risk to 8 with no mitigation. The math says the trouble was worth making. It does not say she made it well. **Lesson: GT tells you whether to fight. It does not tell you how. Lower the denominator before you swing.**

**Field Report B --- Bishop's tool, three years later, same idea, different method.**
He rebuilt Keisha's compliance tool under the $50K efficiency exemption (page 81), ran it in the dormant sandbox (page 119), and authorized the pilot on his own signature as a time-boxed proof of value (page 142). Impact_change: 9 (still two million). Alignment_values: 10 (still believes). Risk: 2 (every step cited a real policy; the cover was airtight). Friction: 3 (he routed around ARB legally, not by hiding). GT = (9 x 10) / (2 + 3) = 90 / 5 = **18.0.** \textcolor{acid}{GREEN}\textcolor{acid}{GREEN}\textcolor{acid}{GREEN}. **Same impact. Same belief. He tripled the coefficient by gutting the denominator. That is the whole craft.**

**Field Report C --- The all-hands manifesto.**
You stand up in the quarterly meeting and tell three hundred people the company has lost its soul. Impact_change: 2 (you changed nothing; you vented). Alignment_values: 9 (you meant every word). Risk: 9 (you are now The Problem). Friction: 5 (nothing moves; you just made yourself radioactive). GT = (2 x 9) / (9 + 5) = 18 / 14 = **1.3.** \textcolor{red}{RED}. It *felt* like courage. It was just expensive noise. Save it.
...
**Benchmarks:**
- \textcolor{acid}{GREEN} I_insurgent >= 5.0 --- Ghost mode. You are solving five times more than you're claiming. The immune system can't see you because there's no profile to target. Maximum survivability, maximum reach.
- \textcolor{amber}{YELLOW} I_insurgent 2.0--4.9 --- Visible insurgent. Effective but exposed. You're building a reputation, which is a target. Acceptable if the reputation buys you cover; dangerous if it buys you envy.
- \textcolor{red}{RED} I_insurgent < 2.0 --- You're a self-promoter with a cause. You claim as much as you solve, or more. The system sees you coming. Bishop calls this "getting fired for being right out loud."
...
Problems solved quietly: 23 (the loopholes, each one a shipped fix). Credit claimed publicly: 0. The index is technically undefined --- you can't divide by zero --- which is exactly the point. **A perfect insurgent leaves no numerator on the trophy and no denominator anywhere.** In practice we score his "claimed" as 1 (one time he let himself be named, to protect Sarah) and call it I = 23. \textcolor{acid}{GREEN}\textcolor{acid}{GREEN}\textcolor{acid}{GREEN}. He is invisible to REAPER because there is nothing to flag.

**Field Report B --- The Growth Hacker from Chapter 5.**
Problems solved quietly: 3 (he did ship some real things). Credit claimed publicly: 40 (the talks, the threads, the "how I 10x'd retention" posts). I = 3/40 = **0.075.** \textcolor{red}{RED}. He is a billboard. The system loves him exactly as long as the numbers hold and erases him the day they don't, because his entire existence is a claim.

**Field Report C --- A mid-level PM, year one.**
Problems solved: 8. Credit claimed: 4. I = 2.0. \textcolor{amber}{YELLOW}. She's good. She's also building a personal brand, which means she's building a target. **The advice: solve four more before you claim one more. Get the ratio to green before someone with a worse ratio decides you're a threat.**
...
```{=latex}
\dichapter{CHAPTER 9}{Calculus for Cowards}{The Economics of Safety}
```
...
\textcolor{acid}{GREEN} **GREEN** --- L_legacy < 1.15 x P. The process is stable. Cost barely compounds. Refinancing saves real money but it's not urgent. Lead with capability, not liability.

\textcolor{amber}{YELLOW} **YELLOW** --- 1.15 x P <= L_legacy < 1.40 x P. The liability is growing faster than inflation. There's a bleeding-neck somewhere upstream (rising volume, rising error rate). Find it. Quantify it. Lead with the daily number.

\textcolor{red}{RED} **RED** --- L_legacy >= 1.40 x P. The status quo is a runaway. Every quarter of delay costs measurably more than the last. This is your easiest pitch and his scariest spreadsheet. Put the three-year total in pink and hand it over.
...
L_legacy = 4.49M x (1.18)³ = 4.49M x 1.643 = **$7.38M.**
That's 1.64 x P. **\textcolor{red}{RED} RED.** The three-year cost of "let's revisit next fiscal year" is $7.38M --- and the daily number ($12,290) is just the first day's installment. Park's eleven-minute no would have cost him $12,290 before lunch.
...
L_legacy = 310K x (1.05)³ = 310K x 1.158 = **$359K.**
That's 1.16 x P. **\textcolor{amber}{YELLOW} YELLOW.** Barely compounding. The liability is real but slow. Don't oversell urgency you can't back --- lead with the route quality, not the apocalypse.
...
L_legacy = 1.2M x (1.22)³ = 1.2M x 1.816 = **$2.18M.**
That's 1.82 x P. **\textcolor{red}{RED} RED.** The CFO budgeted $1.2M and is on the hook for $2.18M of cumulative bleed if he waits three years. Show him the curve, not the slope. Curves scare CFOs. Slopes are abstract.
...
\textcolor{red}{RED} **RED** --- S_fc < 1.0. The pilot does not pay for itself in its own window. You are asking the CFO for faith, which he ran out of in Q1. Shrink the scope, shorten the window, or don't pitch.

\textcolor{amber}{YELLOW} **YELLOW** --- 1.0 <= S_fc < 2.5. The pilot breaks even or modestly profits. Defensible, but a skeptical CFO will haggle every input. Tighten your assumptions until they're conservative enough to embarrass him.

\textcolor{acid}{GREEN} **GREEN** --- S_fc >= 2.5. The pilot pays for itself two-and-a-half times or more, inside the trial. This isn't a purchase. It's free money he's currently leaving on the floor. Sign it on his personal authority. No committee. No fiscal cycle. No zombies. (See Ch 10.)
...
S_fc = (390K − 74K) / 95K = 316K / 95K = **3.33.** **\textcolor{acid}{GREEN} GREEN.**
...
Cost_AI = $19K.
Cost_pilot = $40K.
S_fc = (52K − 19K) / 40K = 33K / 40K = **0.83.** **\textcolor{red}{RED} RED.**
...
Cost_AI = $88K.
Cost_pilot = $120K.
S_fc = (410K − 88K) / 120K = 322K / 120K = **2.68.** **\textcolor{acid}{GREEN} GREEN.**
Pays for itself 2.68x inside the trial. Sign on signing authority. Note: this one runs a 120-day window because storm-season volume front-loads the savings --- match your window to where the pain spikes, not to a round number.
...
2. **Compute L_legacy from outside data.** Pull P from the org chart, job postings, public salary bands, and one number a friendly insider mentions in a hallway. Estimate r honestly from volume growth and error trends. Run it to t=3. Classify \textcolor{acid}{GREEN}/\textcolor{amber}{YELLOW}/\textcolor{red}{RED}.
...
```{=latex}
\dichapter{CHAPTER 10}{The Qualification Protocol}{Avoiding Zombies}
```
...
**Zombie Scorecard (Score)** --- The qualification equation. Pain x 5, Timeline x 3, Budget x 5. Tells you whether the thing in front of you has a pulse before you spend a quarter finding out the hard way.
...
```
Score = (Pain_Owner x 5) + (Timeline x 3) + (Budget x 5)
```
...
- \textcolor{acid}{GREEN} **45--65** --- Live prospect. Pulse confirmed. Spend real resources.
- \textcolor{amber}{YELLOW} **25--44** --- Weak signal. One vital is missing. Run a pulse check before you invest. Find the missing vital or walk.
- \textcolor{red}{RED} **0--24** --- Zombie. No pulse. Do not transfuse. Send the Breakup Email and move your blood somewhere it'll circulate.
...
```
Score = (0 x 5) + (0 x 3) + (0 x 5) = 0
```

\textcolor{red}{RED} 0. A perfect zombie. Looks green in the pipeline, dead on the slab. The correct action is not "nurture." It's the Breakup Email --- a pulse check, not a surrender.
...
```
Score = (4 x 5) + (3 x 3) + (5 x 5) = 20 + 9 + 25 = 54
```

\textcolor{acid}{GREEN} 54. Same logo. Same inbox. The company was never the zombie. **The gatekeeper was.** The Innovation Lead wasn't the company's interest --- he was the company's antibody, the soft tissue at the door that absorbs vendors so the people with real problems never have to.
...
```
Score = (3 x 5) + (1 x 3) + (2 x 5) = 15 + 3 + 10 = 28
```

\textcolor{amber}{YELLOW} 28. The dangerous one. Real enough to hope, dead enough to bleed you. Don't transfuse yet. Run a pulse check: ask her to name the by-when and the consequence. If she can't, the timeline stays 1 and you walk. If she can, you've just promoted a \textcolor{amber}{YELLOW} to a \textcolor{acid}{GREEN} in one question.
...
- \textcolor{acid}{GREEN} **I_v < 1.0** --- More demo than description. The thing exists and you let it speak. Trust-positive.
- \textcolor{amber}{YELLOW} **1.0--3.0** --- Talking more than showing. Either the product isn't ready or you don't trust it to. Either way, fix it.
- \textcolor{red}{RED} **> 3.0** --- Vaporware register. You are Brayden. Stop describing the future and build a noun.
...
\textcolor{red}{RED} Infinite. The mathematical signature of a zombie. Pure description, zero existence. Division by zero is the sound a building makes when it has no product.
...
\textcolor{acid}{GREEN} 0.29. The thing did the thing. Trust didn't need a single transformational anything.
...
\textcolor{red}{RED} 3.67. Beautiful slides. Animated transitions. A vision statement with a colon in it. The prettier the deck, the more you should ask to touch the engine --- because Dorian Gray's portrait was always in the room they didn't show you.
...
1. **SCORE EVERY INBOUND COLD.** Before you take the second meeting, compute the Zombie Scorecard. Pain x 5, Timeline x 3, Budget x 5. Write the number down. If it's under 25, you have a zombie regardless of how good it feels.
...
4. **SEND THE BREAKUP EMAIL TO ALL \textcolor{red}{RED} DEALS.** Four sentences. Acknowledge, decide, unlatch the door, name the specific problem in the unlatch. Send it this week to every dead-scoring deal in your pipeline. Treat replies as pulse readings, not victories.

5. **PROTECT YOUR CALENDAR LIKE BLOOD.** Track hours spent per deal against its score. Any \textcolor{red}{RED} deal still on your calendar is a transfusion into a corpse. Pull the needle.
...
```{=latex}
\dichapter{CHAPTER 11}{Weaponizing Bureaucracy}{The Compliance Shield}
```
...
```
D_audit = (Certifications x Scope) / Auditor_Fear
```
...
**Benchmarks:**
- \textcolor{acid}{GREEN} **GREEN (>= 4.0):** Strong certs, tight scope, calm room. The audit becomes a formality. Forty-five minutes.
- \textcolor{amber}{YELLOW} **YELLOW (1.5--3.9):** You have some certs but the scope is loose or the auditor is nervous. Survivable, but you'll bleed weeks.
- \textcolor{red}{RED} **RED (< 1.5):** Thin certs, unbounded scope, terrified auditor. The audit becomes an interrogation with no exit. Abort and rebuild your shield first.

**Field Report A --- The Naked Startup.** Series A fintech, two engineers, no SOC 2, going into a bank audit. Certifications: 1 (a self-attestation that means nothing). Scope: 2 (the bank scoped the *entire* platform because nothing reassured them). Auditor_Fear: 9 (the auditor's bonus rode on catching problems). D_audit = (1 x 2) / 9 = **0.22.** \textcolor{red}{RED} RED. They spent four months in the audit and lost the deal anyway. The lesson cost them a quarter and a co-founder.

**Field Report B --- The Inherited Shield.** Same startup, eight weeks later. They migrated everything onto a hyperscaler, inherited its SOC 2 controls, completed their own SOC 2 Type II. Certifications: 6. They drew the scope to the data plane only --- one narrow seam. Scope: 8. They pre-shipped the entire control matrix before the meeting, so the auditor walked in calm. Auditor_Fear: 3. D_audit = (6 x 8) / 3 = **16.0.** \textcolor{acid}{GREEN} GREEN. The audit took thirty minutes. Same product. Same code. Different shield.

**Field Report C --- ZERO at MegaCorp.** Certifications: 5 (inherited cloud SOC 2, her own SOC 2 Type II, ISO 27001, a signed HIPAA BAA, PCI-adjacent attestation). Scope drawn to a single integration boundary: 9. Auditor_Fear lowered by a pre-shipped binder and a comparison matrix: 3. D_audit = (5 x 9) / 3 = **15.0.** \textcolor{acid}{GREEN} GREEN. The forty-five-minute slot used twenty-eight minutes. REAPER's flag downgraded to *monitored.*
...
```
Demo_Risk = Real_Records_Exposed x Breach_Cost_per_Record
```
...
```{=latex}
\dichapter{CHAPTER 12}{The Invasion Plan}{From Shadow to System of Record}
```
...
```
T_r = (Volume_traffic x Time_uptime) / Severity_incidents
```
...
**Benchmarks:**
- \textcolor{red}{RED} **RED (T_r < 100):** You're a science project. Not enough live volume, not enough clean time. Stay in the shadow. Do not push for the throne yet --- you'll lose the argument and the access.
- \textcolor{amber}{YELLOW} **YELLOW (T_r 100--10,000):** You've earned a seat at the table, not the head of it. You can win on a Save. You cannot win on a deck.
- \textcolor{acid}{GREEN} **GREEN (T_r > 10,000):** You ARE the lower-risk option. The legacy is now the dangerous incumbent. When the Save lands, the math does your closing for you.

**Field Report Alpha --- ZERO at MegaCorp (this chapter).** Three months of shadow. Volume_traffic = ~14 million events processed read-only. Time_uptime = 92 days clean. Severity_incidents = 0.5 (one near-miss false positive in week two, caught before it fired). T_r = (14,000,000 x 92) / 0.5 = **2.576 billion.** Deep green. When she pushed the alert, nobody asked whether the shadow was reliable. Ninety-two clean days and fourteen million events had already answered that. 

**Field Report Bravo --- the legacy platform, same night.** Volume_traffic = the same 14M events (it saw them too). Time_uptime = 4,015 days (eleven years). Severity_incidents = one supply-chain miss that nearly cost $4.2M, weighted at 100, plus accumulated drift. T_r ~ (14,000,000 x 4,015) / 100 = **562 million.** Half of ZERO's, despite eleven years of runway --- because *one big miss is worth more than a decade of boring.* 

**Field Report Charlie --- the over-eager startup that pushed too early.** Volume_traffic = 40,000 events (mostly synthetic, demo load). Time_uptime = 9 days. Severity = 1 (a false alarm that crashed a staging environment during the pilot). T_r = (40,000 x 9) / 1 = **360,000** --- looks green, but the volume is fake and the uptime is a rounding error. They pushed an "alert" on day nine to look heroic, were wrong, and lost the contract. The lesson: T_r is only real if the volume is real and the time is long. You cannot fake the boring. The boring IS the asset.
...
```{=latex}
\dichapter{CHAPTER 13}{Field Report 01 --- The Fortress Run}{Banking Case Study}
```
...
**Migration Effort^2** --- The squared term in Vendor Gravity. Why incumbents feel immovable even when they're garbage. Pulling a SoR out is exponentially expensive --- which is why you want to *be* the SoR.
...
- **Migration_Effort^2** --- the squared cost of ripping it out: integration points, retraining, risk. Squared because risk compounds.

**Benchmarks:**
\textcolor{acid}{GREEN} F_g < 2 --- incumbent is beatable head-on
\textcolor{amber}{YELLOW} 2 <= F_g < 6 --- do not attack the incumbent; go around it
\textcolor{red}{RED} F_g >= 6 --- the incumbent is load-bearing; coexist or die

**Field Report A --- Goliath's fraud vendor.** Sunk_Cost (normalized to a 1--10 scale across nine years and the capital spend) = 9. Political_Capital = 8 (the SVP who signed it is now the COO). Migration_Effort = 7 -> squared = 49. F_g = (9 x 8) / 49 = **1.47.**
...
**Field Report B --- the SAR drafting tool (no incumbent).** Sunk_Cost = 0 (nobody owns this problem with software). Political_Capital = 1. Migration_Effort = 2 -> squared = 4. F_g = (0 x 1) / 4 = **0.00.** No gravity. No incumbent. An open lane. *This* is where you plant.
...
**Benchmarks:**
\textcolor{acid}{GREEN} V_w >= 4 --- ship it; the wedge will spread on its own
\textcolor{amber}{YELLOW} 1 <= V_w < 4 --- workable, but you'll have to push
\textcolor{red}{RED} V_w < 1 --- the wedge is too slow; the immune system reabsorbs it

**Field Report A --- Sarah's SAR tool.** Pain_acute = 9 (two filings already past deadline; regulatory fines on the clock). Trust_increment = 0.8 (she reviews and signs every narrative --- visible, controllable). Friction_adopt = 1.5 (one read-only connection, no new login, runs inside her existing workflow). V_w = (9 x 0.8) / 1.5 = **4.8.** \textcolor{acid}{GREEN} Ship it.

**Field Report B --- a "fraud platform upgrade" pitch to Mike.** Pain_acute = 3 (the existing vendor mostly works; nothing's on fire). Trust_increment = 0.2 (a platform swap is a leap of faith). Friction_adopt = 9 (procurement, security review, migration, retraining). V_w = (3 x 0.2) / 9 = **0.07.** \textcolor{red}{RED} Dead on arrival. This is the pitch ZERO *didn't* make. GHOST told her she was pitching the wrong drowning, and the math agreed.

**Field Report C --- month 9, expanding into transaction monitoring.** By then the SAR tool had nine months of clean output and Sarah's trust. Pain_acute = 6, Trust_increment = 0.7 (banked from the wedge), Friction_adopt = 3. V_w = (6 x 0.7) / 3 = **1.4.** \textcolor{amber}{YELLOW} Workable --- *because the wedge had already raised the trust term.* The same expansion at month one would have scored 0.4 and died. Trust is a multiplier you earn over time, then spend.
...
**Benchmarks:**
\textcolor{acid}{GREEN} T_r >= 50 --- the tool is load-bearing; it has earned the right to act
\textcolor{amber}{YELLOW} 10 <= T_r < 50 --- trusted, but supervise
\textcolor{red}{RED} T_r < 10 --- not yet; keep it read-only
...
Volume_traffic by then = 380,000 transactions scored. Time_uptime = 14 months. Severity_incidents = 2 (two cosmetic narrative typos in fourteen months, weight 1 each). T_r = (380,000 x 14) / 2 = **2.66 million.** \textcolor{acid}{GREEN}. Off the chart. The tool had earned the right to act.
...
3. **Engineer V_w >= 4.** Maximize acute pain x controllable trust; minimize adoption friction. Read-only, no new login, human signs everything. If V_w < 1, the wedge is too slow --- redesign it.

4. **Keep it read-only until T_r >= 50.** Earn the right to act through volume x uptime. Do not let the tool push the button on day one. Let the *human* push it.
...
```{=latex}
\dichapter{CHAPTER 14}{Field Report 02 --- The Paperclip Maximizer}{Healthcare Case Study}
```
...
**Soul Score** --- S_soul = Σ(Impact_human x Intent_honest). Chapter 5's equation. The only number Dr. Vance actually cares about, though she'd never call it that.
...
**Formula:** V_w = (Pain_acute x Trust_increment) / Friction_adopt
...
**Benchmarks:** \textcolor{acid}{GREEN} V_w > 4.0 (the wedge drives itself) | \textcolor{amber}{YELLOW} 1.5--4.0 (push needed) | \textcolor{red}{RED} < 1.5 (you're selling vision, abort)
...
V_w = (9 x 8) / 4 = **18.0** \textcolor{acid}{GREEN}
...
V_w = (5 x 1) / 10 = **0.5** \textcolor{red}{RED}
...
V_w = (7 x 6) / 3 = **14.0** \textcolor{acid}{GREEN}

### Intel Block 3 --- Soul Score (S_soul)

**Formula:** S_soul = Σ(Impact_human x Intent_honest)
...
**Benchmarks:** \textcolor{acid}{GREEN} S_soul strongly positive and rising | \textcolor{amber}{YELLOW} positive but flat (you've stopped helping people and started helping metrics) | \textcolor{red}{RED} <= 0 (you've become the paperclip maximizer)
...
S_soul contribution = 10 x 1 = **+10** \textcolor{acid}{GREEN}
...
S_soul = 10 x −0.8 = **−8.0** \textcolor{red}{RED}
...
S_soul ~ **+63** \textcolor{acid}{GREEN} and climbing.
...
The leap from $36K to $480K is **13.3x**. You did not sell it. The save sold it. 
...
```{=latex}
\dichapter{CHAPTER 15}{Field Report 03 --- The Propaganda Farm}{The Ethics Case Study}
```
...
**Benchmarks:**
- \textcolor{acid}{GREEN} **GREEN (< 1.0):** The product's beauty roughly matches its engine. Sleep fine.
- \textcolor{amber}{YELLOW} **YELLOW (1.0 -- 5.0):** The portrait is aging faster than the face. Someone should look. They won't, yet.
- \textcolor{red}{RED} **RED (> 5.0):** The interface is a mask. The prettier the feed gets, the worse the engine has to behave to feed it. File the document.
...
\textcolor{red}{RED} RED. Two hundred and five. Not borderline. Not a judgment call. A portrait so rotted it can't be in the same room as a regulator and survive.

**Field Report B --- MirrorMatch, for calibration.** ZERO runs it on her old shop from memory. Loneliness-driven re-engagement revenue: $9M. Trust_eroded: 0.22. Time: 3.0 years. D_gray = (9 / 0.22) x 3 = 40.9 x 3 = **122.** \textcolor{red}{RED} RED. The portrait she fled scored a hundred and twenty-two. She files that number in her own head and says nothing.

**Field Report C --- a clean comparison.** A subscription note-taking app, no ads, no engagement farming. Dark-pattern revenue near zero --- call it $0.1M from a single mildly aggressive cancel-flow. Trust_eroded: 0.95 (almost no measured harm). Time: 4 years. D_gray = (0.1 / 0.95) x 4 = 0.105 x 4 = **0.42.** \textcolor{acid}{GREEN} GREEN. 
...
Not with a manifesto. With a section. "Risk assessment: session-quality and downstream harm." And in that section: the Sidecar's methodology, and the D_gray value, and the \textcolor{red}{RED} RED reading. Cited. Reproducible. Timestamped.
...
**Benchmarks:**
- \textcolor{acid}{GREEN} **GREEN (> 3.0):** High-leverage good trouble. Do it now.
- \textcolor{amber}{YELLOW} **YELLOW (1.0 -- 3.0):** Worth it, but check your exposure.
- \textcolor{red}{RED} **RED (< 1.0):** You're a martyr, not an insurgent. Redesign the approach.

**Field Report A --- the megaphone path (don't).** Suppose SIGNAL quits loud and goes to a reporter. Impact_change = 9 (huge story). Alignment = 8. But Risk = 9 (career incinerated, NDA war, becomes-the-story) and Friction = 7 (legal threats, no internal record). GT = (9 x 8) / (9 + 7) = 72 / 16 = **4.5.** \textcolor{acid}{GREEN} --- but look at that Risk term. High score, high body count. The number's good and the person is gone.

**Field Report B --- the DPIA path (do this).** Same exposure. Impact_change = 8 (the record outlives any single article and is legally weaponized). Alignment = 10 (it's the actual job). Risk = 2 (procedurally correct, protected activity). Friction = 2 (a passenger service, a doc edit). GT = (8 x 10) / (2 + 2) = 80 / 4 = **20.** \textcolor{acid}{GREEN}. Twenty versus four-point-five. The quiet path scores four times higher *and* SIGNAL keeps the keyboard. Good trouble doesn't need a stage. It needs a filing system.

**Field Report C --- the do-nothing path.** Impact_change = 0. GT = 0. \textcolor{red}{RED}. This is the option most senior people pick, and they call it prudence. The math calls it zero.
...
```{=latex}
\dichapter{CHAPTER 16}{Victory Without Fire}{The Morning After}
```
...
Benchmarks:
\textcolor{red}{RED} GT < 1 --- vanity. Loud, risky, helps no one but your ego.
\textcolor{amber}{YELLOW} 1 <= GT < 3 --- real but expensive. Make sure the juice is worth it.
\textcolor{acid}{GREEN} GT >= 3 --- good trouble. Ship it.
...
**Field Report B --- The Layer She Planted.** Impact_change = 1,880,000 people/day getting a tagged, declinable result. Alignment = 1.0 (this is the whole thesis). Risk = 2 (she's a clerk; the change is additive, discoverable, defensible). Friction = 3. GT = (1,880,000 x 1.0) / (2 + 3) = **376,000.** \textcolor{acid}{GREEN}, and then some. 

**Field Report C --- The Daily Compounding.** GT is a rate, not a total. Run the layer 365 days: 686 million human-impressions of *one less piece of garbage* in year one, with Risk and Friction trending toward zero as it normalizes. \textcolor{acid}{GREEN}. 
...
Benchmarks:
\textcolor{red}{RED} < 1 --- you built landfill. The model ships it cheaper tomorrow.
\textcolor{amber}{YELLOW} 1--5 --- defensible for now. Keep the human in the loop.
\textcolor{acid}{GREEN} > 5 --- irreplaceable. This is the currency of the future.

**Field Report A --- The Generic Pitch.** Trust = 2, Substitutability = 0.9. T = 2 x (1/0.9) = **2.2.** \textcolor{amber}{YELLOW}. Fine. Forgettable. The kind of thing every chapter warned you about.

**Field Report B --- The Layer.** Trust = 9 (1.88 million people *feel* the difference, even if they can't name it). Substitutability = 0.1 (PRISM could copy the mechanism but never the intent; intent is the un-copyable part). T = 9 x (1/0.1) = **90.** \textcolor{acid}{GREEN}\textcolor{acid}{GREEN}. 
...
Benchmarks:
\textcolor{red}{RED} < 1 --- you're a brand, not a builder.
\textcolor{amber}{YELLOW} 1--10 --- effective and visible. Watch the ego.
\textcolor{acid}{GREEN} > 10 --- invisible and unstoppable. The system can't fight what it can't credit.
...
```{=latex}
\dichapter{APPENDIX A}{THE INSURGENT'S CHEATSHEET}{All twenty-four equations. Formula, variables, traffic-light benchmarks, and the one move each number tells you to make. Rip this out. Tape it to the wall.}
```
...
Benchmark key: \textcolor{acid}{GREEN} GREEN = go / safe | \textcolor{amber}{YELLOW} YELLOW = caution / investigate | \textcolor{red}{RED} RED = stop / redesign / abort.
...
**Benchmarks:** \textcolor{acid}{GREEN} < 1 | \textcolor{amber}{YELLOW} 1--5 | \textcolor{red}{RED} > 5 (you are landfill the moment tooling catches up).
...
**Benchmarks:** \textcolor{acid}{GREEN} long (months->years) | \textcolor{amber}{YELLOW} weeks | \textcolor{red}{RED} < 6 months.
...
**Benchmarks:** \textcolor{acid}{GREEN} < 2 | \textcolor{amber}{YELLOW} 2--5 | \textcolor{red}{RED} > 5 (all talk, no ship).
...
**Benchmarks:** \textcolor{acid}{GREEN} < 20 (movable) | \textcolor{amber}{YELLOW} 20--80 | \textcolor{red}{RED} > 80 (do NOT pitch replacement).
...
**Benchmarks:** read the curve, not a point. \textcolor{acid}{GREEN} flat | \textcolor{amber}{YELLOW} bending | \textcolor{red}{RED} hockey stick.
...
**Benchmarks:** \textcolor{acid}{GREEN} low | \textcolor{amber}{YELLOW} mid | \textcolor{red}{RED} high (obscure + terrifying = nobody will touch it).
...
**Benchmarks:** \textcolor{acid}{GREEN} < 0.8 | \textcolor{amber}{YELLOW} 0.8--1.0 | \textcolor{red}{RED} > 1.0 (the team is underwater).
**Move:** don't pitch the blueprint to a drowning person. Be the bucket.

### 8. Consensus Paralysis Factor --- $F_{cp}$
$$F_{cp} = \frac{n(n-1)}{2}$$
**Benchmarks:** \textcolor{acid}{GREEN} <= 6 (n<=4) | \textcolor{amber}{YELLOW} 10--21 (n 5--7) | \textcolor{red}{RED} > 28 (n >= 8).
...
**Benchmarks:** explodes as noise -> infinity, **only if** context is real. \textcolor{red}{RED} context_depth ~ 0 = a Top-10 listicle.
...
**Benchmarks:** \textcolor{acid}{GREEN} high trust / low substitutability (a sommelier's pick) | \textcolor{red}{RED} -> 0 (AI-washable).
...
**Benchmarks:** \textcolor{acid}{GREEN} < 100 | \textcolor{amber}{YELLOW} 100--1000 | \textcolor{red}{RED} > 1000 (career risk > any upside; walk).
...
**Benchmarks:** \textcolor{acid}{GREEN} before 11 AM | \textcolor{amber}{YELLOW} post-lunch | \textcolor{red}{RED} after 2 PM (default answer is "no").
...
**Benchmarks:** \textcolor{acid}{GREEN} < 10 | \textcolor{amber}{YELLOW} 10--50 | \textcolor{red}{RED} > 50 (the portrait is becoming visible; collapse incoming).
...
**Benchmarks:** \textcolor{acid}{GREEN} net positive | \textcolor{red}{RED} net negative (fix the product before you read another page).
...
**Benchmarks:** \textcolor{acid}{GREEN} < 1 (ship it) | \textcolor{amber}{YELLOW} 1--10 (feature-flag it) | \textcolor{red}{RED} > 10--100 (canary first / do not deploy).
...
**Benchmarks:** \textcolor{acid}{GREEN} > 50 (viral) | \textcolor{amber}{YELLOW} 10--50 | \textcolor{red}{RED} < 1 (it's a platform pretending to be a wedge).
...
**Benchmarks:** \textcolor{acid}{GREEN} > 5 (go) | \textcolor{amber}{YELLOW} 2--5 (find a Shadow Sponsor) | \textcolor{red}{RED} < 1 (vanity --- wrong terrain).
...
**Benchmarks:** \textcolor{acid}{GREEN} -> infinity (max power) | \textcolor{red}{RED} -> 0 (you're building a LinkedIn, not a movement).
...
**Benchmarks:** read the 3/5/10-year projection. \textcolor{red}{RED} anything that doubles inside 5 years.
...
**Benchmarks:** \textcolor{acid}{GREEN} > 5 (pays for itself) | \textcolor{acid}{GREEN}\textcolor{acid}{GREEN} > 10 (CFO is negligent not to buy) | \textcolor{red}{RED} < 1 (don't pitch the CFO yet).
...
**Benchmarks (max 65):** \textcolor{acid}{GREEN} 45--65 (live) | \textcolor{amber}{YELLOW} 25--44 (one vital missing) | \textcolor{red}{RED} 0--24 (corpse --- send the Breakup Email).
...
**Benchmarks:** \textcolor{acid}{GREEN} < 5 | \textcolor{amber}{YELLOW} 5--10 | \textcolor{red}{RED} > 10 (slides, not software).
...
**Benchmarks:** \textcolor{acid}{GREEN} > 50 (rubber stamp) | \textcolor{amber}{YELLOW} 20--50 | \textcolor{red}{RED} < 5 (rejection).
...
**Benchmarks:** \textcolor{acid}{GREEN} > 10,000 (ready for primary) | \textcolor{acid}{GREEN}\textcolor{acid}{GREEN} > 100,000 (System of Record) | \textcolor{red}{RED} < 100 (stay in shadow).
...
```{=latex}
\dichapter{APPENDIX B}{THE INFILTRATION CANVAS}{One page per engagement. Fill it out before you take the meeting, not after you lose it. Eight panels, in order. Skip one and the fortress finds the gap you didn't.}
```
...
- **Vendor Gravity** $F_g$: sunk cost x political capital / migration effort^2. -> Replace or sidecar?
...
- **Zombie Score** = (Pain_Owner x5) + (Timeline x3) + (Budget_Code x5). Total ___ / 65.
...
- **One-line read:** \textcolor{acid}{GREEN}/\textcolor{amber}{YELLOW}/\textcolor{red}{RED} --- _if \textcolor{red}{RED}, send the Breakup Email and move my blood somewhere it circulates._
...
- **Audit Defense** $D_{audit}$ = certs x scope / auditor_fear = ___
...
- **Wedge Velocity** $V_w$ = pain x trust / friction = ___ (need > 10; adopt-friction = 1)
...
- **Good Trouble** $GT$ = (impact x alignment) / (risk + friction) = ___ (need > 2)
...
```{=latex}
\dichapter{APPENDIX C}{THE COMPANION APP --- DIGITAL INSURGENCY LIVE}{The field manual is a book. This is the book that listens. A tactical companion that runs the equations against your real meetings, in real time, and tells you when to pivot, when to push, and when to abort.}
```
