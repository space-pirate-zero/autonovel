...
**Wedge-to-Platform** — The path from a $36K billing contract to a $480K clinical platform. The whole point. The billing was never the product. The billing was the door.
...
**MAIN POINT:** You don't argue your way across the Hippocratic Blockade. You let the boring bot save a life on its own, in writing, with a doctor's name on the catch. Then the line in concrete is the doctor's to erase, not yours. The wedge is billing. The platform is the patient. The bridge between them is one man's knee.
...
ZERO sits cross-legged on the cold floor of a borrowed colo cage, three counties from St. Mary's, jacked into the Mesh through a terminal she built from parts. The cyan at the base of her skull pulses, slow, a tide she stopped trying to read. Above her the racks blink amber and green. She's not supposed to be here. She's read-only. That's the whole point.
...
Then she does the thing that costs her. She doesn't send it to Vance. She doesn't send it to anyone with a stethoscope she can take credit with. She routes it through the billing coordinator — a nurse named Patrice who reviews every MediMind flag at 7 AM with her coffee — and lets it become Patrice's catch, then Vance's call.
...
$36K pilot. Below the malpractice radar entirely, because billing is not medicine, and Dr. Vance drew that line herself. You did not draw it. She did. That's what makes it hold.
...
Same platform. Same pitch you couldn't give in month zero. Now it drives itself, because the wedge built the road.
...
The leap from $36K to $480K is **13.3×**. You did not sell it. The save sold it. Dr. Vance walked through the door on her own legs, and she got to be right about both things at once: right that you don't let algorithms touch medicine carelessly, AND right that this one caught what her own pre-op process missed.
...
3. **Route the catch to a trusted human.** When the sidecar flags something real, surface it through the institution's own staff so the catch belongs to a nurse, a coordinator, a doctor — never the vendor. Theft claims credit. Selection gives it away.
...
There's this thought experiment — paperclip maximizer. You build an AI, you tell it to make paperclips, you walk away, and it turns your bones into paperclips because you forgot to tell it to stop. Everybody laughs at it like it's science fiction. I've shipped it. Twice. Not a robot apocalypse. A growth target. You point a system at "recovered dollars" and you walk away, and one Tuesday it quietly suppresses the flag that would've saved a man's knee, because the save doesn't bill and the lawsuit might. Nobody decides this. Nobody twirls a mustache. A PM moves a card to "won't fix" and goes to lunch. That's the apocalypse. It wears a lanyard and it's polite.
