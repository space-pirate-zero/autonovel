...
- **Known build bug #1 (fixed in build, not committed):** the Dockerfiles use Node 20 with an unpinned pnpm; corepack now pulls pnpm 11.13.0 which needs Node 22's `node:sqlite` → `pnpm install` crashes. **Fix: pin pnpm** (`corepack prepare pnpm@10.15.1 --activate`) or bump base to `node:22-alpine`. *(Applied to a throwaway build copy this session; needs committing to the Dockerfiles.)*
- **Known build bug #2 (blocks prod build):** `next build` fails with `Type error: Argument of type 'PostHogInterface' is not assignable to parameter of type 'PostHog'` in `packages/analytics/src/posthog.ts` — posthog-js type drift (dev tolerates it, prod build does not). **Fix:** a type cast/signature fix in `posthog.ts`, or set `typescript.ignoreBuildErrors: true` in each site's `next.config`. Two Cloud Build attempts this session failed here; the live sites still serve the **April 2026** build.
